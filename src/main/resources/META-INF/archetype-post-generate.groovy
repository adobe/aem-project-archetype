...
/**
 * Renames and deletes frontend related files as necessary (non-SPA only)
 */
def cleanUpFrontendModule(frontendModules, optionFrontendModule, rootPom, rootDir, appsFolder, confFolder, configFolder, contentFolder) {
    // Delete unwanted frontend modules and normalize "general" to ui.frontend
    frontendModules.each { def moduleName ->
        // Clean up POM file entries like <module>ui.frontend.general</module>
        removeModule(rootPom, 'ui.frontend.' + moduleName)

        def moduleDir = new File(rootDir, "ui.frontend.${moduleName}")
        if (optionFrontendModule == moduleName) {
            // Selected frontend => rename to ui.frontend
            if (moduleDir.exists()) {
                assert moduleDir.renameTo(new File(rootDir, "ui.frontend"))
            }
        } else {
            // Non-selected frontend => delete if it exists
            if (moduleDir.exists()) {
                assert moduleDir.deleteDir()
            }
        }
    }
}
