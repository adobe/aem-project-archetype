...
function getEntrypoints(assetManifestPath) {
  if (!fs.existsSync(assetManifestPath)) {
    throw Error(
      `Cannot determine entrypoints: No asset manifest found at path ${symbol_dollar}{assetManifestPath}`
    );
  }
  ...
  if (!('entrypoints' in manifestContent)) {
    throw Error(
      `Cannot determine entrypoints: Missing "entrypoints" key in ${symbol_dollar}{assetManifestPath}`
    );
  }
